...
from project6.birdsong.models import Note
...
class ProfileView(DetailView):
    ...
    def get_object(self):
        return Profile.objects.get(pk=self.kwargs.get('pk', None))

    def get_context_data(self, *args, **kwargs):
        context = super(ProfileView, self).get_context_data(*args, **kwargs)
        context['object_list'] = Note.objects.filter(profile=self.get_object())
        return context
...
class ProfileUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self, *args, **kwargs):
        return reverse('profiles:detail', args=(self.get_object().pk, ))
